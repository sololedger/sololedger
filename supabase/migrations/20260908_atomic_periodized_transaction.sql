-- supabase/migrations/20260908_atomic_periodized_transaction.sql
--
-- Gör periodisering atomisk:
--   - periodiseringsverifikation 31/12
--   - vändningsverifikation på future_date
--   - båda verifikationsnumren
--   - samtliga journalrader
-- skapas i EN PostgreSQL-transaktion.
--
-- Om något steg misslyckas rullas hela periodiseringen tillbaka, inklusive
-- båda verifikationsnumren.
...
REVOKE ALL ON FUNCTION public.book_periodized_transaction_atomic(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.book_periodized_transaction_atomic(jsonb) FROM anon;
