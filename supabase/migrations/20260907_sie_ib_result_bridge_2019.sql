-- supabase/migrations/20260907_sie_ib_result_bridge_2019.sql
--
-- Denna migration ersätter föregående version av public.import_sie_batch(jsonb)
-- och lägger till en syntetisk 2019-rad endast när #IB inte balanserar själv
-- men differensen exakt motsvaras av #RES -1.
--
-- Bygger vidare på 20260907_sie_opening_balance_duplicate_guard.sql.
--
-- Funktionell ändring:
--   * import_sie_batch() accepterar previous_year_result_balances i payloaden
--   * server-side IB-kontroll stämmer av sum(#IB 0) + sum(#RES -1) = 0
--   * om #RES -1 saknas behålls den tidigare strikta kontrollen sum(#IB 0) = 0
--   * befintligt dubblettskydd, atomisk import och övrig validering lämnas oförändrade
--
